...
function auth(reg=false){app.innerHTML=`<div class="auth"><div class="box"><h1>🎮 Robom Games</h1><p class="sub">${reg?"Создай аккаунт":"Войди в аккаунт"}</p>${reg?'<input id="name" class="input" placeholder="Имя" autocomplete="name">':""}<input id="email" class="input" placeholder="Почта" type="email" autocomplete="${reg?"email":"username"}"><input id="pass" class="input" placeholder="Пароль" type="password" autocomplete="${reg?"new-password":"current-password"}"><label class="small" style="display:flex;align-items:center;gap:8px;margin:4px 0 10px;cursor:pointer"><input id="showPass" type="checkbox" onchange="document.querySelector('#pass').type=this.checked?'text':'password'"> Показать пароль</label><button class="btn" onclick="submitAuth(${reg})">${reg?"Зарегистрироваться":"Войти"}</button><button class="link" onclick="auth(${!reg})">${reg?"Уже есть аккаунт? Войти":"Нет аккаунта? Регистрация"}</button></div></div>`}

function readStoredAccount(){let data=null;try{data=JSON.parse(localStorage.getItem(KEY)||"null")}catch(e){}if(data&&data.email)return data;try{data=JSON.parse(localStorage.getItem("robom_account")||"null")}catch(e){}if(data&&data.email)return data;return null}

function submitAuth(reg){const emailEl=document.querySelector("#email"),passEl=document.querySelector("#pass");const e=emailEl.value.trim().toLowerCase(),p=passEl.value;if(!e||!p)return alert("Заполни почту и пароль");
if(reg){const nameEl=document.querySelector("#name");if(p.length<4)return alert("Пароль должен быть минимум 4 символа");u={name:nameEl.value.trim()||"Игрок",email:e,pass:p,tokens:100,robom:0};save();home();return}
const candidates=[];if(u&&u.email)candidates.push(u);const stored=readStoredAccount();if(stored)candidates.push(stored);
let found=null;for(const account of candidates){if(String(account.email||"").trim().toLowerCase()===e&&String(account.pass??account.password??"")===p){found=account;break}}
if(!found){for(const account of candidates){if(String(account.email||"").trim().toLowerCase()===e&&String(account.pass??account.password??"").trim()===p.trim()){found=account;break}}}
if(!found)return alert("Неверная почта или пароль. Нажми «Показать пароль» и проверь ввод.");
u={name:found.name||"Игрок",email:String(found.email).trim().toLowerCase(),pass:String(found.pass??found.password??p),tokens:Number(found.tokens)||0,robom:Number(found.robom)||0};save();home()}
