...
function head(){return `<div class="head"><div><b>Привет, ${u.name}! 👋</b></div><div class="wallet"><span class="coin">🪙 Токены: ${u.tokens}</span><span class="coin">💠 Робом: ${u.robom}</span><button class="exchangeBtn" onclick="exchange()">Обмен</button><button class="exit" onclick="stopTimers();u=null;localStorage.removeItem(KEY);auth()">Выйти</button></div></div>`}
function home(){stopTimers();app.innerHTML=`<div class="wrap">${head()}<h1>Главное меню</h1><p class="sub">Выбирай игру и зарабатывай валюту.</p><div class="grid"><div class="game"><div><h2>🏐 Волейбол</h2><p>После старта каждые 30 секунд +1 Токен.</p></div><button class="btn" onclick="volley()">Играть</button></div><div class="game"><div><h2>⚽ Набивание мяча</h2><p>После старта каждую секунду +1 Токен.</p></div><button class="btn" onclick="kick()">Играть</button></div><div class="game soon"><div><h2>🏀 Баскетбол</h2><p>Новая игра уже готовится.</p></div><button class="btn" disabled>Скоро будет</button></div><div class="game"><div><h2>🛠️ Для разработчиков</h2><p>Инструменты разработчика.</p></div><button class="btn" onclick="developerLogin()">Открыть</button></div></div><p class="small recovery">Потеряли аккаунт? Напишите на <a href="mailto:gamedevelover@tokman.net">gamedevelover@tokman.net</a></p></div>`}
